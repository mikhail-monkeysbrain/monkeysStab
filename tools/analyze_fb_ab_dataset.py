...
paired=[r for r in a if r.get("ab_fb_valid",0)>0.5 and r.get("ab_robust_valid",0)>0.5]
...
print(f"CSV: {path}")
print(f"rows={len(rows)}  A_valid={len(a)}  paired_A_B_C={len(paired)} ({100*len(paired)/max(1,len(a)):.2f}%)")
