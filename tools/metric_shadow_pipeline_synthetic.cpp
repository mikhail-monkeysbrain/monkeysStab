...
int main(){
  ...
  for(int k=-1;k<=3;k++){
    metric_shadow::TimedAttitude g{};
    g.valid=true;
    g.sample_ns=t0+k*10000000LL;
    g.roll=0.02; g.pitch=-0.03; g.yaw=0.15;
    ah.push_back(g);
  }
  const auto a0s=metric_shadow::interpolateAttitude(ah,t0,25.0);
  const auto a1s=metric_shadow::interpolateAttitude(ah,t1,25.0);

  std::deque<LunaSample> rh;
  for(int k=-1;k<=3;k++) rh.push_back(LunaSample{0.20,100,t0+k*10000000LL});
  const auto r0s=metric_shadow_sync::interpolateRange(rh,t0,25.0,15.0);
  const auto r1s=metric_shadow_sync::interpolateRange(rh,t1,25.0,15.0);

  if(!a0s.valid||!a1s.valid||!r0s.valid||!r1s.valid){
    std::cerr<<"FAIL sync gate\n"; return 2;
  }

  const metric_shadow::Attitude a0=a0s.attitude;
  const metric_shadow::Attitude a1=a1s.attitude;
  ...
  auto broken=ah;
  broken.clear();
  broken.push_back(ah.front());
  broken.push_back(ah.back());
  const auto bad=metric_shadow::interpolateAttitude(broken,t0,15.0);
