...
def main():
 ap=argparse.ArgumentParser();ap.add_argument("--camera",default="/dev/video0");ap.add_argument("--per-zone",type=int,default=4);a=ap.parse_args()
 ...
 counts=np.zeros((3,3),int); descs=[[[] for _ in range(3)] for _ in range(3)];meta=[];last=0.;total=9*a.per_zone
 print(f"SERIES_C balanced 3x3: target {a.per_zone}/zone = {total} frames. AUTO capture. Q/ESC stop.")
 while counts.sum()<total:
  ...
  if cc is not None and nc>=8:
   ...
   if counts[zy,zx]>=a.per_zone:reason="zone full"
   else:
    old=descs[zy][zx]
  ...
  for y in (160,320):cv2.line(vis,(0,y),(639,y),(255,255,255),1)
  for yy in range(3):
   for xx in range(3):
