...
def _record_sample(sample):
    with _lock:
        ...
        if h is None:return
        row={"wall_time":time.time()};row.update(sample)
        csv.writer(h).writerow([row.get(k,"") for k in RUN_RECORD_COLUMNS]);h.flush()

def run_record_status():
    with _lock:return {"recording":_run_record_handle is not None,"started_wall":_run_record_started_wall or None,"pending_name":_run_record_pending is not None}

def start_run_record():
    global _run_record_handle,_run_record_tmp,_run_record_started_wall,_run_record_pending
    with _lock:
        if _run_record_handle is not None:return {"ok":True,**run_record_status()}
        if _run_record_pending is not None:raise RuntimeError("Сначала сохраните имя предыдущего прогона")
        RUN_RECORD_DIR.mkdir(parents=True,exist_ok=True);stamp=time.strftime("%Y%m%d_%H%M%S")
        ...
        csv.writer(_run_record_handle).writerow(RUN_RECORD_COLUMNS);_run_record_started_wall=time.time()
    log_event("INFO","Запись прогона начата");return {"ok":True,**run_record_status()}

def stop_run_record():
    global _run_record_handle,_run_record_pending
    with _lock:
        if _run_record_handle is None:return {"ok":True,**run_record_status()}
        _run_record_handle.close();_run_record_handle=None;_run_record_pending=_run_record_tmp
    log_event("INFO","Запись прогона остановлена — ожидается имя");return {"ok":True,**run_record_status()}

def finalize_run_record(name):
    global _run_record_pending,_run_record_tmp,_run_record_started_wall
    with _lock:
        src=_run_record_pending
        if src is None or not Path(src).exists():raise RuntimeError("Нет остановленного прогона для сохранения")
        clean=re.sub(r'[\\/:*?"<>|]+',"_",str(name or "").strip())
        if not clean:raise ValueError("Введите название прогона")
        stamp=time.strftime("%Y%m%d_%H%M%S",time.localtime(_run_record_started_wall or time.time()))
        dst=RUN_RECORD_DIR/(stamp+"_"+clean[:100]+".csv");n=2
        while dst.exists():dst=RUN_RECORD_DIR/(stamp+"_"+clean[:100]+f"_{n}.csv");n+=1
        os.replace(src,dst);_run_record_pending=None;_run_record_tmp=None;_run_record_started_wall=0.0
    log_event("INFO","Прогон сохранён: "+dst.name);return {"ok":True,"name":dst.name}

def list_run_records():
    RUN_RECORD_DIR.mkdir(parents=True,exist_ok=True);out=[]
    for p in RUN_RECORD_DIR.glob("*.csv"):
        if p.name.startswith(".active_"):continue
        try:st=p.stat();out.append({"name":p.name,"size":st.st_size,"mtime":st.st_mtime})
        except OSError:pass
...
<div id="runNameModal" style="display:none;position:fixed;inset:0;background:#000a;z-index:100;align-items:center;justify-content:center"><div class="card" style="width:min(520px,92vw);padding:18px"><h3>Название прогона</h3><input id="runNameInput" type="text" maxlength="100" placeholder="Например: X+500_return_01" style="width:100%;margin:10px 0"><div id="runNameMsg" style="color:#ff7b86"></div><div class="actionBar"><button class="btn green" onclick="saveRunName()">СОХРАНИТЬ ПРОГОН</button></div></div></div>
<div id="logsModal" style="display:none;position:fixed;inset:0;background:#000a;z-index:100;align-items:center;justify-content:center"><div class="card" style="width:min(720px,94vw);max-height:80vh;overflow:auto;padding:18px"><h3>Сохранённые прогоны</h3><div id="logsList"></div><div class="actionBar" style="margin-top:14px"><button class="btn green" onclick="downloadSelectedLogs()">СКАЧАТЬ ВЫБРАННЫЕ</button><button class="btn" onclick="$('logsModal').style.display='none'">ЗАКРЫТЬ</button></div></div></div>
...
async function refreshRunRecordStatus(){try{let j=await api('/api/run-record/status');runRecording=!!j.recording;let b=$('runRecordBtn');if(b)b.textContent=runRecording?'■ ОСТАНОВКА ПРОГОНА':'● ЗАПИСЬ ПРОГОНА';if(j.pending_name)$('runNameModal').style.display='flex'}catch(e){}}
async function toggleRunRecord(){try{if(runRecording){await api('/api/run-record/stop',{method:'POST'});runRecording=false;$('runNameModal').style.display='flex';$('runNameInput').focus()}else{await api('/api/run-record/start',{method:'POST'});runRecording=true}await refreshRunRecordStatus()}catch(e){alert(e.message)}}
async function saveRunName(){let name=$('runNameInput').value.trim();if(!name){$('runNameMsg').textContent='Введите название';return}try{await api('/api/run-record/finalize',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});$('runNameModal').style.display='none';$('runNameInput').value=''}catch(e){$('runNameMsg').textContent=e.message}}
async function openLogsModal(){$('logsModal').style.display='flex';let j=await api('/api/run-logs');$('logsList').innerHTML=(j.logs||[]).map(x=>'<label style="display:grid;grid-template-columns:24px 1fr auto;gap:8px;padding:8px"><input type="checkbox" class="runLogCheck" value="'+escapeHtml(x.name)+'"><span>'+escapeHtml(x.name)+'</span><span>'+((x.size||0)/1024).toFixed(1)+' КБ</span></label>').join('')||'Логов пока нет'}
function downloadSelectedLogs(){let n=[...document.querySelectorAll('.runLogCheck:checked')].map(x=>x.value);if(!n.length){alert('Выберите хотя бы один лог');return}location.href='/api/run-logs/download?names='+encodeURIComponent(n.join('\n'))}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            elif p=="/api/run-record/status": self.send_json(run_record_status())
            elif p=="/api/run-logs": self.send_json({"logs":list_run_records()})
            elif p=="/api/run-logs/download":
                names=parse_qs(urlparse(self.path).query).get("names",[""])[0].split("\n");avail={x["name"]:RUN_RECORD_DIR/x["name"] for x in list_run_records()};files=[avail[n] for n in names if n in avail]
                if not files:raise ValueError("Не выбраны логи")
                if len(files)==1:data=files[0].read_bytes();filename=files[0].name;ctype="text/csv; charset=utf-8"
                else:
                    bio=io.BytesIO()
...
if __name__=="__main__":
    ...
    try:
        ...
        # runtime. start_runtime() already performs the required runtime-side
        # startup checks, so do not add a second preflight/test cycle here.
        try:
            start_runtime()
            print("Flight runtime: АВТОЗАПУСК ГОТОВ",flush=True)
        except Exception as e:
            # Keep the Web UI available so the operator can see the exact
            # startup error and retry manually after fixing the cause.
