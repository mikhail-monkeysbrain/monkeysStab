...
# Actual locally installed panel uses finalX/finalY/finalZ, not fusionX/fusionY/fusionZ.
...
repls={
 " if($('finalX'))$('finalX').textContent='—';":" if($('finalX'))$('finalX').textContent=finalX!=null?fmt(finalX,1)+' мм':'—';",
 " if($('finalY'))$('finalY').textContent='—';":" if($('finalY'))$('finalY').textContent=finalY!=null?fmt(finalY,1)+' мм':'—';",
 " if($('finalZ'))$('finalZ').textContent='—';":" if($('finalZ'))$('finalZ').textContent=finalZ!=null?fmt(finalZ,1)+' мм':'—';",
...
for old,new in repls.items():
    assert old in s, 'anchor not found: '+old
...
for old_note in [
 ...
 'Камера: WORKED5 X/Y. IMU: независимое dead reckoning. Итоговая fusion XYZ пока не реализована и не подменяется EKF-данными.'
    ...
    if old_note in s:
        s=s.replace(old_note,'Камера: WORKED5 X/Y. IMU: независимое dead reckoning. Итог v1: WORKED5 X/Y + IMU Z; горизонтальный IMU DR пока показан отдельно и не усредняется без валидации.',1)
        break

p.write_text(s)
print('OK: actual finalX/finalY/finalZ panel populated; correction row follows final XYZ.')
