...
struct ImuStats { Stats ax,ay,az; };\nstatic void rot_xyz(double r,double p,double y,double x,double yy,double z,double &ox,double &oy,double &oz){double cr=cos(r),sr=sin(r),cp=cos(p),sp=sin(p),cy=cos(y),sy=sin(y);ox=cy*cp*x+(cy*sp*sr-sy*cr)*yy+(cy*sp*cr+sy*sr)*z;oy=sy*cp*x+(sy*sp*sr+cy*cr)*yy+(sy*sp*cr-cy*sr)*z;oz=-sp*x+cp*sr*yy+cp*cr*z;}
...
static void report(const char* name,const ImuStats&s,double ar,double ap,double ayaw,double trx,double try_){
...
int main(){try{
 ...
 Stats roll,pitch,yaw; ImuStats hi,i1,i2,i3; double trimx=-0.03428453207,trimy=-0.0220823437;
 ...
 while(std::chrono::steady_clock::now()<end){pollfd q{fd,POLLIN,0};if(poll(&q,1,100)<=0)continue;auto n=read(fd,b,sizeof(b));for(ssize_t j=0;j<n;j++){if(!mavlink_parse_char(MAVLINK_COMM_0,b[j],&m,&st)||m.sysid!=sys)continue;
  if(m.msgid==MAVLINK_MSG_ID_ATTITUDE){mavlink_attitude_t a{};mavlink_msg_attitude_decode(&m,&a);roll.add(a.roll);pitch.add(a.pitch);yaw.add(a.yaw);}
