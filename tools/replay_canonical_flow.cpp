...
struct Step { bool valid=false; int reason=0,features=0,tracked=0,inliers=0; double du_px=0,dv_px=0,du=0,dv=0,scale=0,yaw=0; };
...
static Step estimate(const cv::Mat& prev,const cv::Mat& curr,double dt,const cv::Mat& K,const cv::Mat&D){
 ...
 cv::Mat sol;if(!cv::solve(A,bb,sol,cv::DECOMP_SVD)){o.reason=5;return o;}o.du=sol.at<double>(0);o.dv=sol.at<double>(1);o.scale=sol.at<double>(2)/dt;o.yaw=sol.at<double>(3)/dt;o.du_px=med(dup);o.dv_px=med(dvp);o.valid=std::hypot(o.dv/dt,-o.du/dt)<4.0;if(!o.valid)o.reason=6;return o;
}
int main(int argc,char**argv){
 ...
 std::ofstream out(d+"/canonical_cpp_replay.csv");out<<"frame,dt_s,replay_valid,prod_valid,replay_features,prod_features,replay_tracked,prod_tracked,replay_inliers,prod_inliers,replay_du_norm,prod_du_norm,replay_dv_norm,prod_dv_norm\n";
 cv::Mat prev;long long pts=0;size_t exactF=0,exactT=0,exactI=0,n=0;double maxdu=0,maxdv=0;
 for(auto&m:ms){std::vector<uchar>jpg(m.size);fb.read((char*)jpg.data(),jpg.size());if((size_t)fb.gcount()!=jpg.size()){std::cerr<<"short mjpgbin at frame "<<m.frame<<"\n";return 3;}cv::Mat cur=cv::imdecode(jpg,cv::IMREAD_GRAYSCALE);if(cur.empty()){std::cerr<<"decode failed "<<m.frame<<"\n";return 3;}if(prev.empty()){prev=cur;pts=m.cam;continue;}double dt=(m.cam-pts)*1e-9;Step s=estimate(prev,cur,dt,K,D);auto&p=prod[m.frame];n++;exactF+=s.features==p.fe;exactT+=s.tracked==p.tr;exactI+=s.inliers==p.in;maxdu=std::max(maxdu,std::abs(s.du-p.du));maxdv=std::max(maxdv,std::abs(s.dv-p.dv));out<<std::setprecision(12)<<m.frame<<","<<dt<<","<<s.valid<<","<<p.valid<<","<<s.features<<","<<p.fe<<","<<s.tracked<<","<<p.tr<<","<<s.inliers<<","<<p.in<<","<<s.du<<","<<p.du<<","<<s.dv<<","<<p.dv<<"\n";prev=cur;pts=m.cam;}
