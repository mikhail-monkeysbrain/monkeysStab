...
int main(int argc,char**argv){
 if(argc<2||argc>3){std::cerr<<"usage: canonical_cpp_replay DATASET_DIR [focal_scale]\n";return 2;}cv::setNumThreads(1);std::string d=argv[1]; double fs=(argc==3)?std::stod(argv[2]):.931;
 ...
 cv::Mat K=(cv::Mat_<double>(3,3)<<568.53170752165227*fs,0,315.98271077441063,0,569.68005562865858*fs,239.88148589100641,0,0,1);cv::Mat D=(cv::Mat_<double>(1,5)<<.073569192194028493,-.095253893789117,-.010810530757187299,-.0022843373576970235,.082177400802757483);
 std::string out_path=d+"/canonical_cpp_replay.csv"; if(argc==3){std::ostringstream q;q<<d<<"/canonical_cpp_replay_fs_"<<std::fixed<<std::setprecision(6)<<fs<<".csv";out_path=q.str();}std::ofstream out(out_path);out<<"frame,dt_s,replay_valid,prod_valid,replay_features,prod_features,replay_tracked,prod_tracked,replay_inliers,prod_inliers,replay_du_norm,prod_du_norm,replay_dv_norm,prod_dv_norm,sim_rms_norm,aff_rms_norm,aff_improvement_pct,aff_tx,aff_ty,aff_a11,aff_a12,aff_a21,aff_a22\n";
