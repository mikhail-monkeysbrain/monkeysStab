...
DATASET_DIR="${MONKEYS_DATASET_DIR:-$HOME/monkeysStab_datasets/${STAMP}_EXPERIMENT_AB_RAW}"
...
import socket,sys
try:
 s=socket.create_connection(("127.0.0.1",5760),timeout=.15); s.close(); sys.exit(0)
...
import socket,sys
try:
 s=socket.create_connection(("127.0.0.1",5760),timeout=.1); s.close(); sys.exit(0)
except OSError: sys.exit(1)
PY
    sleep .1
  done
...
if [[ ! -x "$BIN" || "$SRC" -nt "$BIN" ]]; then
  ...
  g++ -std=c++17 -O2 -DNDEBUG -pthread -Wno-address-of-packed-member \
    ...
    "$SRC" -o "$BIN" $(pkg-config --libs opencv4) -lpthread >"$RUN_DIR/build.log" 2>&1
fi
...
r=d.get('feature_roi',[.20,.32,.80,.90]); print(d.get('focal_scale',.931),*r,d.get('max_features',500))
PY
)
...
exec "$BIN" "$CAMERA" "$LUNA" "$FC" "$CSV" "$CAMERA_YAML" "$FOCAL" \
  ...
  --diag-range-z-m "$RZ" --blind4-cli --dataset-dir "$DATASET_DIR"
