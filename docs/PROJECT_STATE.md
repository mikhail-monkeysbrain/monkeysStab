...
### 6.14 End-to-end latency текущего OPTICAL_FLOW — PASS (2026-09-20)

Использован уже существующий контрольный прогон `20260920_153029_OPTICAL_FLOW`;
новый физический прогон не требовался.

Production CSV уже содержит `camera_ts_ns`, `camera_dequeue_ns`,
`flow_send_ns` и `frame_pipeline_latency_ms`, а runtime отбрасывает
измерения старше 80 ms.

Результат по 1796 интервалам:
- capture -> dequeue: median 8.787 ms, p95 14.349 ms, p99 17.109 ms,
  max 84.067 ms;
- dequeue -> send: median 8.830 ms, p95 10.673 ms, p99 11.909 ms,
  max 22.162 ms;
- capture -> send: median 18.068 ms, p95 24.197 ms, p99 26.942 ms,
  max 93.485 ms;
- фактически отправленные в FC: 1795/1796;
- для отправленных capture -> send: median 18.068 ms, p95 24.182 ms,
  p99 26.905 ms, max 30.221 ms;
- только один интервал превысил 40/60/80 ms; он достиг 93.485 ms и не был
  отправлен благодаря существующему 80 ms freshness gate.

Вердикт: патологического backlog на реально отправляемых измерениях в этом
контроле нет. Единственный крупный выброс локализован до camera dequeue и уже
отсекается существующим freshness gate. Отдельный новый latency-механизм или
ещё один физический прогон сейчас не нужны.

Ограничение остаётся архитектурным: AP_OpticalFlow_MAV датирует измерение
временем получения MAVLink-пакета, поэтому типичный возраст ~18 ms не равен
