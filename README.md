...
Во время работы monkeysStab входы **RC6** и **RC8** используются как аппаратная кнопка
сброса рабочей опорной точки:

- переход RC6 **или** RC8 выше **1700 мкс** принимает текущую позицию за `0/0/0`;
- новое событие разрешается после возврата **обоих** каналов ниже **1500 мкс**;
