...
Положение OV9281 и TF-Luna больше не нужно вписывать в несколько файлов вручную. Для этого есть отдельная GUI-утилита (графическое окно):
...
После сохранения утилита:

- записывает значения в `config/mount_geometry.json`;
- обновляет положение камеры в `config/ov9281_current_mount.yaml`;
- показывает готовые значения `FLOW_POS_X/Y/Z` и `RNGFND1_POS_X/Y/Z`, которые нужно внести в Mission Planner;
- показывает разнос камеры и дальномера по X/Y/Z.

**FC автоматически не перепрограммируется.** После изменения геометрии значения из GUI нужно внести в Mission Planner, нажать **Write Params** и перезагрузить FC. Следующий запуск `scripts/run.sh` проверит, совпадают ли параметры FC с сохранённой геометрией.

Файл `config/mount_geometry.json` является единым источником текущей геометрии для launcher и geometry audit (проверки положения датчиков).
